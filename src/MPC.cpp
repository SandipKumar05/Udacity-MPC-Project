...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 0; i < N; i++) {
      fg[0] += 60*CppAD::pow(vars[cte_start + i], 2);
    ...
    for (int i = 0; i < N - 1; i++) {
      ...
      // try to add a cost function for taking turn smoothly
      fg[0] += 20*CppAD::pow(vars[delta_start + i] * vars[v_start+i]*3, 2);
      // fg[0] += CppAD::pow((vars[delta_start + i + 1] - vars[delta_start + i])*(vars[v_start+i + 1] -vars[v_start+i]), 2);
    }
